drop function if exists public.apply_work_order_action(
  uuid,
...
create function public.apply_work_order_action(
...
begin
  ...
  where work_order.id = p_work_order_id
    and work_order.status = p_expected_status
  returning * into v_work_order;

  if not found then
    raise exception 'Work order is no longer in the expected status.';
  end if;
