...
export class ChatRepository {
  ...
  async read(q: ChatQuery): Promise<Evidence> {
    ...
    switch (q.topic) {
      ...
      case "work_orders":
        table = "work_orders";
        columns = `id,incident_id,status,assigned_at,updated_at,respond_due_at,resolve_due_at,incidents!inner(ticket_number,location_label,asset_name,description,urgency_verified)${this.role === "technician" ? ",work_order_assignees!inner(technician_id)" : ""}`;
      ...
      facts: clean({
        ...
        scope:
          q.topic === "work_orders"
            ...
            : q.topic === "dispatch_queue"
              ? "คิวรอจัดสรร"
              : "ข้อมูลตามสิทธิ์บัญชีที่เข้าสู่ระบบ",
      }),
      sources,
    };
  }
