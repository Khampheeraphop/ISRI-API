...
function assertThrows(callback: () => void, expectedType: new (...args: never[]) => Error) {
  try {
    callback();
  } catch (error) {
...
Deno.test("dispatcher can approve or return only work waiting for review", () => {
...
Deno.test("workflow rejects invalid role, state, and missing mandatory note", () => {
